...
语音识别录入（说一句话自动填充名称/分类/数量/过期时间）明确列为暂不做，留作后续独立迭代，不要在其他改动里顺带加。原因：跟项目"纯离线"定位有冲突（语音转文字依赖系统能力，不同机型效果不一致），且语义解析只能做规则匹配，工作量和这批其他改动不是一个量级，需要单独评估验证。
...
| id | Long | 主键自增 |
| name | String | 食物名称，必填 |
| category | FoodCategory? | 分类枚举，**选填**（用户明确要求分类不再强制） |
| addedDate | LocalDate | 录入日期，默认今天 |
| expiryDate | LocalDate | 过期日期，必填，表单支持两种输入方式：选具体日期 / 选"N天后"相对输入，提交时统一换算成绝对日期存库 |
| quantity | Double? | 重量/个数，选填 |
| quantityUnit | QuantityUnit? | 数量单位（枚举：克/千克/斤/个/盒等），选填，表单默认预填"斤" |

分类枚举固定集合（不做用户自定义分类，保持简单）：蔬菜水果、肉类、海鲜、蛋奶、速冻食品、饮品、调味品、其他。

因为category改为可选且是全新迭代阶段（还没有真实用户数据），Room数据库版本升级用`fallbackToDestructiveMigration()`，不必单独写migration脚本。
...
- 分类筛选放在可展开的抽屉侧滑（`ModalNavigationDrawer`），默认收起，不占用列表页横向空间，点击入口按钮展开
- 列表页顶部增加统计栏：食材总数 / 即将过期数（含3天内） / 已过期数
- 滑动删除改为二次确认：滑动触发`AlertDialog`确认框，确认后才真正删除；不再保留撤销Snackbar（二次确认和撤销是重复保险，二选一即可）
- 食物图片：每个分类固定配一张写实风格代表图（不是按具体食物单独配图，因为食物名称是用户自由输入，无法预先枚举），素材来源和处理方式见交付说明

## 设计系统

- 遵循Material 3 Design规范，用Compose的`MaterialTheme`承载色彩、字体、形状token，不手写魔法数字
- 色彩：清新时尚的绿色主题，语义色区分正常/即将过期/已过期三种状态
